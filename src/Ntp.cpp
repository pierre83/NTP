...
uint32_t NTPClient::getEpoch(const char* NTPServer)
{
	...
	if ( ret == SUCCESS ) {		// Récupération de l'@IP du DNS déclaré dans Ethernet.begin(xxx))
		...
		if ( ret == SUCCESS ) {
			//Serial.print("DNS success");	Serial.println(IPAddress(NTPAddress));
                ...
                if ( result == SUCCESS ) {
                    ...
                    while ( millis() < stopWait ) {
                        ...
                        if ( packetSize >= NTP_PACKET_SIZE ) {
							// We've got something, is it from who we expected ?
							if ( ( nUdp.remoteIP() == NTPAddress ) || ( nUdp.remotePort() == NTP_PORT ) ) {
